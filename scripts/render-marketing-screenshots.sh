...
CONFIG="${CONFIG:-Debug}"
SCHEME="${SCHEME:-Foil}"
OUTPUT_DIR="${MARKETING_SCREENSHOT_OUTPUT_DIR:-${ROOT_DIR}/docs/evidence/marketing-screenshots/latest}"
...
"${EXECUTABLE}" --render-marketing-screenshots --snapshot-output "${OUTPUT_DIR}"

if [[ ! -f "${OUTPUT_DIR}/receipt.json" ]]; then
  echo "ERROR: screenshot receipt missing at ${OUTPUT_DIR}/receipt.json" >&2
  exit 1
fi

png_count="$(find "${OUTPUT_DIR}" -maxdepth 1 -name '*.png' -type f | wc -l | tr -d ' ')"
if [[ "${png_count}" != "4" ]]; then
  echo "ERROR: expected 4 PNG screenshots, found ${png_count}" >&2
  find "${OUTPUT_DIR}" -maxdepth 1 -type f | sort >&2
  exit 1
fi

echo "Rendered marketing screenshots to ${OUTPUT_DIR}"
