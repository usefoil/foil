...
final class AppStateTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        testDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("FoilAppStateTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: testDirectory, withIntermediateDirectories: true)
        KeychainHelper.storageDirectoryOverride = testDirectory
        KeychainHelper.serviceOverride = "com.neonwatty.Foil.app-state-tests.\(UUID().uuidString)"
    ...
    private static func httpResponse(statusCode: Int, url: URL = URL(string: "http://127.0.0.1:8080/v1/models")!) -> HTTPURLResponse {
        HTTPURLResponse(
            url: url,
    ...
    func testAccessibilityRecoveryDetailExplainsStaleIdentityRepairPath() {
        ...
        XCTAssertTrue(detail.contains("Return to Foil"))
    ...
    func testVocabularyLocalCorrectionScopePersistsUpdatesAndDeletesAtomically() throws {
        let fileURL = testDirectory.appendingPathComponent("local-corrections.json")
        let store = LocalCorrectionStore(fileURL: fileURL)
        let state = AppState(localCorrectionStore: store)
        ...
        _ = try state.setVocabularyCorrectionLocalScope(id: correction.id, groupID: "agents")
        _ = try state.setLocalCorrectionsEnabled(true)
        XCTAssertEqual(state.localCorrectionSnapshot.rules.count, 1)

        let reloaded = AppState(localCorrectionStore: store)
        XCTAssertEqual(reloaded.transcriptProcessingMode, .raw)
        XCTAssertTrue(reloaded.localCorrectionSnapshot.isEnabled)
        XCTAssertEqual(reloaded.localCorrectionRule(forVocabularyCorrectionID: correction.id)?.group, "agents")
        XCTAssertEqual(
            reloaded.previewLocalCorrections("use super base", activeGroupID: "agents").text,
            "use Supabase"
        )
        XCTAssertEqual(
            reloaded.previewLocalCorrections("use super base", activeGroupID: "human").text,
            "use super base"
        )

        XCTAssertNotNil(
            reloaded.updateVocabularyCorrection(
                id: correction.id,
                writtenAs: "cloud code",
                correctVersion: "Claude Code"
            )
        )
        XCTAssertEqual(
            reloaded.previewLocalCorrections("cloud code", activeGroupID: "agents").text,
            "Claude Code"
        )
    ...
    func testVocabularyDeleteFailurePreservesCorrectionAndStoredRule() throws {
        let fileURL = testDirectory.appendingPathComponent("local-corrections-delete-failure.json")
        let writableStore = LocalCorrectionStore(fileURL: fileURL)
        let state = AppState(localCorrectionStore: writableStore)
        let correction = try XCTUnwrap(
            state.addVocabularyCorrection(writtenAs: "super base", correctVersion: "Supabase")
        )
        _ = try state.setVocabularyCorrectionLocalScope(id: correction.id, groupID: nil)
        ...
        let failingStore = LocalCorrectionStore(
            fileURL: fileURL,
            atomicWriter: { _, _ in throw AppStateSimulatedWriteFailure() }
        )
        let reloaded = AppState(localCorrectionStore: failingStore)

        XCTAssertFalse(reloaded.deleteVocabularyCorrection(id: correction.id))
    ...
    func testVocabularyLocalCorrectionValidationErrorIsVisibleAndLeavesPriorRulesActive() throws {
        let store = LocalCorrectionStore(
            fileURL: testDirectory.appendingPathComponent("local-corrections-validation.json")
        )
        let state = AppState(localCorrectionStore: store)
        ...
        let second = try XCTUnwrap(
            state.addVocabularyCorrection(writtenAs: "SUPER BASE", correctVersion: "Different")
        )
        _ = try state.setVocabularyCorrectionLocalScope(id: first.id, groupID: nil)

        XCTAssertThrowsError(
            try state.setVocabularyCorrectionLocalScope(id: second.id, groupID: nil)
        )
        XCTAssertTrue(
            state.localCorrectionPersistenceError?.contains("Ambiguous local correction aliases") == true
        )
        XCTAssertEqual(state.localCorrectionSnapshot.rules.count, 1)
        XCTAssertEqual(try store.load().rules.count, 1)
    }

    func testCorruptLocalCorrectionFileLoadsDisabledWithoutOverwritingSource() throws {
        let fileURL = testDirectory.appendingPathComponent("local-corrections-corrupt.json")
        let corrupt = Data("not-json".utf8)
        try corrupt.write(to: fileURL)

        let state = AppState(localCorrectionStore: LocalCorrectionStore(fileURL: fileURL))

        XCTAssertFalse(state.localCorrectionSnapshot.isEnabled)
        XCTAssertTrue(state.localCorrectionSnapshot.rules.isEmpty)
        XCTAssertNotNil(state.localCorrectionPersistenceError)
    ...
    func testDefaultTranscriptionProviderIsGroq() {
        ...
        XCTAssertEqual(provider.chatCompletionsEndpoint.absoluteString, "https://api.groq.com/openai/v1/chat/completions")
        XCTAssertEqual(provider.modelsEndpoint.absoluteString, "https://api.groq.com/openai/v1/models")
    }
    ...
    func testCustomOpenAICompatibleProviderPersists() {
        ...
        XCTAssertEqual(reloaded.selectedTranscriptionProviderPresetID, .customOpenAICompatible)
        XCTAssertEqual(reloaded.selectedTranscriptionProviderID, .openAICompatible)
        XCTAssertEqual(provider.id, .openAICompatible)
        XCTAssertEqual(provider.displayName, "Custom OpenAI-compatible")
    ...
    func testLocalWhisperPresetBuildsExpectedProvider() {
        ...
        XCTAssertEqual(state.selectedTranscriptionProviderID, .openAICompatible)
        XCTAssertEqual(provider.displayName, "Local whisper.cpp")
        XCTAssertEqual(provider.transcriptionModel, "whisper-1")
        XCTAssertEqual(provider.audioTranscriptionsEndpoint.absoluteString, "http://127.0.0.1:8080/v1/audio/transcriptions")
    ...
    func testLegacyOpenAICompatibleProviderMigratesToCustomPreset() {
        ...
        UserDefaults.standard.removeObject(forKey: "transcriptionProviderPreset")
        UserDefaults.standard.set("http://localhost:9000/v1", forKey: "customTranscriptionBaseURL")
        UserDefaults.standard.set("tiny", forKey: "customTranscriptionModel")

        let state = AppState()

        XCTAssertEqual(state.selectedTranscriptionProviderPresetID, .customOpenAICompatible)
        XCTAssertEqual(state.selectedTranscriptionProviderID, .openAICompatible)
        XCTAssertEqual(state.selectedTranscriptionProvider.audioTranscriptionsEndpoint.absoluteString, "http://localhost:9000/v1/audio/transcriptions")
        XCTAssertEqual(state.selectedTranscriptionProvider.transcriptionModel, "tiny")
    }

    func testPresetSwitchingUpdatesProviderNameModelAndCleanupAvailability() {
        let state = AppState()
        state.transcriptProcessingMode = .cleanUp

        XCTAssertEqual(state.selectedTranscriptionProvider.displayName, "Groq")
    ...
    func testProviderConnectionInvalidBaseURLFailsBeforeNetwork() async {
        let state = AppState()
        state.selectedTranscriptionProviderPresetID = .customOpenAICompatible
        state.customTranscriptionBaseURL = "file:///tmp/whisper"
        let service = TranscriptionService(transport: StubTransport { request in
            XCTFail("Invalid URL should not make a request: \(String(describing: request.url))")
            return (Data(), Self.httpResponse(statusCode: 500))
        })
        ...
        XCTAssertEqual(
            ...
            .failed("Invalid base URL. Use an http:// or https:// URL.")
        )
    }

    func testProviderConnectionReportsReachableModelAvailable() async {
        let state = AppState()
        state.selectedTranscriptionProviderPresetID = .localWhisperCPP
        let service = TranscriptionService(transport: StubTransport { request in
            (
    ...
    func testProviderConnectionReportsMissingModel() async {
        let state = AppState()
        state.selectedTranscriptionProviderPresetID = .localWhisperCPP
        let service = TranscriptionService(transport: StubTransport { request in
            (
        ...
        XCTAssertEqual(
            state.providerConnectionTestState,
            .failed("Server reachable, but model whisper-1 was not listed.")
        )
    }

    func testProviderConnectionReportsUnreachableServer() async {
        let state = AppState()
        state.selectedTranscriptionProviderPresetID = .localWhisperCPP
...
private struct AppStateSimulatedWriteFailure: Error {}
