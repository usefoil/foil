...
struct GroqTalkE2EConfig {
    var audioPath = "GroqTalk/e2e-test-audio.wav"
...
enum GroqTalkE2EError: Error, CustomStringConvertible {
...
enum GroqTalkE2E {
    ...
    private static func parseConfig() throws -> GroqTalkE2EConfig {
        var config = GroqTalkE2EConfig()
        ...
        while let argument = arguments.first {
            ...
            switch argument {
            case "--audio":
                guard let value = arguments.first else { throw GroqTalkE2EError.missingValue(argument) }
                arguments.removeFirst()
            ...
            default:
                throw GroqTalkE2EError.unknownArgument(argument)
            }
        }
    ...
    private static func run(config: GroqTalkE2EConfig) async throws -> String {
        guard let apiKey = config.apiKey?.trimmingCharacters(in: .whitespacesAndNewlines),
              !apiKey.isEmpty else {
            throw GroqTalkE2EError.missingAPIKey
        }

        let audioURL = URL(fileURLWithPath: config.audioPath)
        guard FileManager.default.fileExists(atPath: audioURL.path) else {
            throw GroqTalkE2EError.invalidAudioPath(config.audioPath)
        }
        guard let format = AudioFormat(rawValue: audioURL.pathExtension.lowercased()) else {
            throw GroqTalkE2EError.invalidFormat(config.audioPath)
        }

        let provider: TranscriptionProvider
        if config.provider == "openai-compatible" {
            guard let baseURL = URL(string: config.baseURL) else {
                throw GroqTalkE2EError.invalidBaseURL(config.baseURL)
            }
        ...
        guard missingWords.count <= config.maxMissingWords else {
            throw GroqTalkE2EError.transcriptMismatch(transcript: transcript, missingWords: missingWords)
        }
    ...
    private static func withTimeout<T>(
        ...
        try await withThrowingTaskGroup(of: T.self) { group in
            ...
            group.addTask {
                ...
                throw GroqTalkE2EError.timedOut(seconds)
            }
