...
render-marketing-screenshots: build
	APP_PATH="$(APP_PATH)" CONFIG="$(CONFIG)" SCHEME="$(SCHEME)" scripts/render-marketing-screenshots.sh
...
test-live-openai-installed:
	scripts/run-installed-openai-transcription-smoke.sh
...
test-paste-real-installed:
	swift tests/test_app_mock_async_paste.swift

test-queued-paste-compatibility:
	scripts/run-queued-paste-compatibility-smoke.sh

test-production-queued-paste-compatibility:
	scripts/run-queued-paste-compatibility-smoke.sh --installed-app
...
test-fresh-user-tcc-qa-script:
	scripts/test-fresh-user-tcc-qa.sh

test-cleanup-quality:
	swift tests/test_cleanup_quality.swift --require-live-provider

test-live-audio-cleanup-quality:
	scripts/run-live-audio-cleanup-quality.sh
