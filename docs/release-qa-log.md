...
| TextEdit queued-paste row | `tests/test_queued_paste_compatibility.swift` | FAIL / PERMISSION-LIMITED | Foil recorded `QueuedPaste.enqueue: status=pending target=TextEdit` and `QueuedPaste.deliver`, but text did not land in the TextEdit target. Rerun after refreshing Accessibility/Input Monitoring for `/Applications/Foil.app`. |
| Browser queued-paste row | `tests/test_queued_paste_compatibility.swift` | FAIL / PERMISSION-LIMITED | Foil recorded `QueuedPaste.enqueue: status=pending target=Google Chrome` and `QueuedPaste.deliver`, but text did not land in the Chrome textarea. Rerun after refreshing Accessibility/Input Monitoring for `/Applications/Foil.app`. |
