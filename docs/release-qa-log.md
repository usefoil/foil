...
| Chrome safety update | `scripts/run-queued-paste-compatibility-smoke.sh` | UPDATED | Default wrapper no longer kills Chrome, closes Chrome tabs, opens browser queued-smoke tabs, or runs `make test-cross-app`. Browser queued-paste coverage is opt-in with `--include-browser`; the older cross-app gate is opt-in with `--include-cross-app` for idle desktops. |
