...
| Browser queued-paste row | `tests/test_queued_paste_compatibility.swift` | PASS | Trusted-app rerun artifact `/tmp/foil-queued-paste-compatibility-20260528-081907`: `Google Chrome pid=76811`, title `Foil Queued Chrome Target - Google Chrome - Jeremy`, queued delivery passed without closing Chrome tabs or quitting Chrome. |
| Unavailable-target fallback row | `tests/test_queued_paste_compatibility.swift` | PASS | Queued TextEdit target, quit TextEdit before delivery, observed clipboard fallback, and verified the clipboard contained the queued transcript text. |
