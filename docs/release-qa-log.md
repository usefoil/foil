...
| Chrome safety update | `scripts/run-queued-paste-compatibility-smoke.sh` | UPDATED | Default wrapper no longer kills Chrome, closes Chrome tabs, or runs `make test-cross-app`. The older cross-app gate is now opt-in with `--include-cross-app` for idle desktops. |
