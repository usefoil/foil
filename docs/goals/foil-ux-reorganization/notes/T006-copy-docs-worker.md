...
- Regular troubleshooting copy explains the stale Foil Accessibility row repair path without developer commands.
...
```text
rg -n "first-run setup|Add API Key|prepare-local-permissions-qa|prepare-local-permissions-qa-check|old Foil row|Local Permission State Repair" README.md
```
