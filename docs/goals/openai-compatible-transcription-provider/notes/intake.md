...
Foil can keep using Groq by default, but can also target a configurable OpenAI-compatible transcription endpoint. The implementation is proven with deterministic unit/UI coverage and an opt-in local Whisper-compatible E2E path using the bundled short WAV clip.
...
- Local E2E proof: bundled `Foil/e2e-test-audio.wav` transcribes through a tiny Whisper-compatible server with at least 8/9 expected words and recorded latency metrics.
