...
- Updated `make test` and `make qa` to force `RUN_LIVE_GROQ_TESTS=0` and skip `GroqTalkTests/LiveGroqIntegrationTests`.
...
- Updated PR/merge-group CI unit tests to set `RUN_LIVE_GROQ_TESTS=0` and skip `GroqTalkTests/LiveGroqIntegrationTests`.
...
- `GROQ_API_KEY=stale RUN_LIVE_GROQ_TESTS=1 make test`
  - PASS
  - `** TEST SUCCEEDED **`
  - xcresult: `Test-GroqTalk-2026.05.24_17-25-06--0700.xcresult`
...
- `make -n test-live-groq`
  - PASS
  - Expands to only `GroqTalkTests/LiveGroqIntegrationTests` after the explicit env setup/restore wrapper.
