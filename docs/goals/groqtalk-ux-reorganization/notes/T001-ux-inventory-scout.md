...
- `docs/superpowers/specs/2026-05-19-groqtalk-ux-reorganization-design.md`
...
- `GroqTalkTests/AppStateTests.swift`: setup readiness, setup check transitions, accessibility recovery copy, session presentation, async paste persistence, mock transcription persistence.
- `GroqTalkTests/RecordingFlowTests.swift`: session presentation and menu bar icon behavior around setup/not-ready states.
- `GroqTalkTests/HotkeyMonitorTests.swift`: hotkey lifecycle, affected only if hotkey control wiring changes.
- `GroqTalkTests/PasteControllerTests.swift`: async paste behavior should remain unchanged after control relocation.

Likely UI-test anchors if explicitly authorized later:

- `GroqTalkUITests/GroqTalkUITests.swift`: setup panel identifiers, setup check action, seeded setup states, mock transcription toggle, async paste toggle, live microphone smoke.
...
- The setup flow should probably be the first Worker slice because it addresses the original permission-order confusion and several existing source gaps are localized to `OnboardingView`, `ApiKeySetupView`, `GroqTalkApp`, `AppState`, and tests.
