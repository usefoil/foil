...
```text
xcodebuild test -scheme GroqTalk -configuration Debug -destination 'platform=macOS' -only-testing:GroqTalkTests/AppStateTests
```
...
```text
rg -n "make prepare-local-permissions-qa" GroqTalk GroqTalkTests
```

Result: only negative assertions in `GroqTalkTests/AppStateTests.swift`; no app UI source references.
