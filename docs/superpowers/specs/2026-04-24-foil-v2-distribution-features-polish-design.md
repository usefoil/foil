# Foil v2: Distribution, Features & Polish

**Goal:** Make Foil discoverable and installable by strangers — polished app icon, clean README, new user-facing features (language hint, history popover), and automated DMG + Homebrew distribution with code signing and notarization.
...
- The 1024x1024 PNG gets placed into `Foil/Assets.xcassets/AppIcon.appiconset/` with the corresponding `Contents.json` updated
...
3. **Install**:
   - Homebrew: `brew tap neonwatty/tap && brew install --cask foil`
...
- **`FoilApp`**: Pass `appState.selectedLanguage` through to the `transcribe()` call.
...
- Modified: `FoilApp.swift` — wire up retry action from the popover
...
**Entitlements:** The existing `Foil.entitlements` already has `com.apple.security.device.audio-input`. No additional entitlements needed — TextInserter uses CGEvent (not AppleScript), so no automation entitlement is required.
...
**DMG contents:** Foil.app + symbolic link to /Applications, with drag-to-install layout.

**Notarization of DMG:**
1. Build and sign the `.app` (above)
2. Create DMG with `create-dmg`
3. Submit DMG to Apple: `xcrun notarytool submit Foil.dmg --apple-id ... --team-id ... --password ... --wait`
4. Staple the ticket: `xcrun stapler staple Foil.dmg`
...
**Naming convention:** `Foil-<version>-macos.dmg` (e.g., `Foil-1.1.0-macos.dmg`)
...
**Update strategy:** After each release, update the cask formula with the new version and SHA256. This can be done manually at first, or automated with a GitHub Action in the tap repo that listens for new releases on `neonwatty/foil`.
