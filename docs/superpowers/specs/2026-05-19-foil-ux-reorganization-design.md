...
Foil's menu bar popover, setup status, and Settings window all expose too many decisions at once. The crowded layout makes first-run setup brittle and makes the app look like it is still missing permissions even after macOS has the permission enabled.
...
- If macOS has a stale Accessibility identity, the UI explains removing the old Foil entry and reopening the app.
...
- `Foil/AppState.swift` only if state presentation needs small support changes
