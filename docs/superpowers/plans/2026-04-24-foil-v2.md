...
```
Foil/
├── FoilApp.swift              # Modify: wire history, retry, timers, format param
...
Replace `FoilTests/AppStateTests.swift` with:
...
```bash
cd /Users/neonwatty/Desktop/foil && xcodegen generate 2>&1 | tail -1 && xcodebuild test -scheme Foil -configuration Debug -destination 'platform=macOS' 2>&1 | tail -10
```
...
Replace `Foil/AppState.swift` with:
...
**Files:**
- Create: `Foil/TranscriptionHistory.swift`
- Create: `FoilTests/TranscriptionHistoryTests.swift`
...
```bash
cd /Users/neonwatty/Desktop/foil
git add Foil/TranscriptionHistory.swift FoilTests/TranscriptionHistoryTests.swift Foil.xcodeproj/
...
Replace `Foil/TextInserter.swift` with:
...
```bash
cd /Users/neonwatty/Desktop/foil && xcodebuild -scheme Foil -configuration Debug -destination 'platform=macOS' build 2>&1 | tail -3
```
...
- Modify: `FoilTests/TranscriptionServiceTests.swift`

- [ ] **Step 1: Write failing tests for format-aware multipart body**

Replace `FoilTests/TranscriptionServiceTests.swift` with:
...
Replace `Foil/TranscriptionService.swift` with:
...
```bash
cd /Users/neonwatty/Desktop/foil && xcodebuild test -scheme Foil -configuration Debug -destination 'platform=macOS' 2>&1 | tail -10
```
...
```bash
cd /Users/neonwatty/Desktop/foil
git add Foil/TranscriptionService.swift FoilTests/TranscriptionServiceTests.swift
...
**Files:**
- Modify: `Foil/AudioRecorder.swift`
...
Replace `Foil/AudioRecorder.swift` with:
...
final class AudioRecorder {
    ...
    private let bufferQueue = DispatchQueue(label: "com.neonwatty.foil.audiobuffers")
...
```bash
cd /Users/neonwatty/Desktop/foil && xcodegen generate 2>&1 | tail -1 && xcodebuild -scheme Foil -configuration Debug -destination 'platform=macOS' build 2>&1 | tail -3
```
...
**Files:**
- Modify: `Foil/HotkeyMonitor.swift`

- [ ] **Step 1: Implement updated HotkeyMonitor with dual strategy**

Replace `Foil/HotkeyMonitor.swift` with:
...
Replace `Foil/MenuBarView.swift` with:
...
### Task 8: FoilApp — Wire Everything Together
...
- [ ] **Step 1: Implement updated FoilApp**
    ...
    private func startHotkeyMonitorWithRetry() {
        ...
        Task {
            ...
            if hotkeyMonitor.start() {
                ...
                appState.showError("Failed to start — try restarting Foil")
            }
        }
    }
}
...
```bash
cd /Users/neonwatty/Desktop/foil
git add Foil/FoilApp.swift Foil.xcodeproj/
...
```bash
cd /Users/neonwatty/Desktop/foil && make install
```
...
5. Check file sizes in `/tmp/foil-*` — M4A and MP3 should be much smaller than WAV
