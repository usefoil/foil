...
enum DiagnosticLog {
    ...
    private static let queue = DispatchQueue(label: "com.foil.diagnostic-log")
    ...
    static func write(_ message: String) {
        ...
        NSLog("[\(AppBrand.name)] %@", redactedMessage)
    ...
    static func setupReportText(
        ...
        return [
            ...
            "- Accessibility: \(diagnosticDescription(for: appState.accessibilityState))",
            "- Microphone: \(diagnosticDescription(for: appState.microphoneState))",
            "- Input Monitoring: not directly detectable by \(AppBrand.name); check macOS Privacy & Security if hotkeys do not arrive.",
    ...
    private static var isEnabled: Bool {
        ...
        return ProcessInfo.processInfo.environment["FOIL_DIAGNOSTICS"] != "0"
    }
    ...
    private static func localModelPathDescription(for appState: AppState) -> String {
        ...
        return "Not stored by \(AppBrand.name); whisper.cpp model files are managed by the local server."
    }
